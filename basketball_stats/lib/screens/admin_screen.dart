...
void _showActaDigital(BuildContext context) {
  _showModal(context, title: 'Acta Digital', child: const _ActaDigitalForm());
}

class _ActaDigitalForm extends StatefulWidget {
  const _ActaDigitalForm();
  @override
  State<_ActaDigitalForm> createState() => _ActaDigitalFormState();
}

class _ActaDigitalFormState extends State<_ActaDigitalForm> with SingleTickerProviderStateMixin {
  ...
  // Stats controllers: matchId_playerId -> {pts, reb, ast, min}
  ...
  Future<void> _loadMatches() async {
    final matches = await Supabase.instance.client
        .from('matches')
        .select('id, round, home_team_id, away_team_id, home_team:teams!matches_home_team_id_fkey(name), away_team:teams!matches_away_team_id_fkey(name)')
        .eq('status', 'finished')
        .order('match_date', ascending: false);
    setState(() { _matches = List<Map<String, dynamic>>.from(matches); _loadingMatches = false; });
  }

  Future<void> _onMatchSelected(Map<String, dynamic> match) async {
    ...
    final players = await Supabase.instance.client.from('players').select('id, first_name, last_name, number').inFilter('team_id', [homeId, awayId]);
    final home = (players as List).where((p) => p['team_id'] == homeId || _playersHome.any((h) => h['id'] == p['id'])).toList();
    // Re-fetch separated
    final homePlayers = await Supabase.instance.client.from('players').select('id, first_name, last_name, number').eq('team_id', homeId).order('number');
    final awayPlayers = await Supabase.instance.client.from('players').select('id, first_name, last_name, number').eq('team_id', awayId).order('number');

    // Init controllers for each player
    for (final p in [...homePlayers, ...awayPlayers]) {
      final pid = p['id'] as String;
      _controllers[pid] ??= {
        'min': TextEditingController(),
        'pts': TextEditingController(),
        'reb': TextEditingController(),
        'ast': TextEditingController(),
        'tap': TextEditingController(),
        'rob': TextEditingController(),
        'fal': TextEditingController(),
      };
    }
  ...
  Widget _playerStatRow(Map<String, dynamic> player) {
      ...
      Padding(
        ...
        child: Row(children: [
          Container(
            ...
            child: Center(child: Text('#${player['number']}', style: const TextStyle(color: AppTheme.primary, fontSize: 11, fontWeight: FontWeight.w800))),
          ),
          const SizedBox(width: 10),
          Text('${player['first_name']} ${player['last_name']}', style: const TextStyle(color: AppTheme.textPrimary, fontSize: 13, fontWeight: FontWeight.w600)),
        ]),
      ),
      Row(children: [
        _statField('MIN', ctrls['min']!),
        const SizedBox(width: 6),
        _statField('PTS', ctrls['pts']!),
        const SizedBox(width: 6),
        _statField('REB', ctrls['reb']!),
        const SizedBox(width: 6),
        _statField('AST', ctrls['ast']!),
        const SizedBox(width: 6),
        _statField('TAP', ctrls['tap']!),
        const SizedBox(width: 6),
        _statField('ROB', ctrls['rob']!),
        const SizedBox(width: 6),
        _statField('FAL', ctrls['fal']!),
      ]),
  ...
  Widget _statField(String label, TextEditingController ctrl) {
    return Expanded(child: Column(children: [
      Text(label, style: const TextStyle(color: AppTheme.textSecondary, fontSize: 9, fontWeight: FontWeight.w700)),
  ...
  Widget build(BuildContext context) {
    ...
    if (_selectedMatch == null) {
      return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Text('Seleccioná el partido', style: TextStyle(color: AppTheme.textSecondary, fontSize: 13)),
        const SizedBox(height: 12),
        ..._matches.map((m) {
          final homeName = (m['home_team'] as Map)['name'] as String;
          final awayName = (m['away_team'] as Map)['name'] as String;
          return GestureDetector(
            onTap: () => _onMatchSelected(m),
            child: Container(
              ...
              child: Row(children: [
                Container(
                  ...
                  decoration: BoxDecoration(color: AppTheme.primaryDim, borderRadius: BorderRadius.circular(6)),
                  child: Text('F${m['round']}', style: const TextStyle(color: AppTheme.primary, fontSize: 11, fontWeight: FontWeight.w700)),
                ),
                const SizedBox(width: 12),
                Expanded(child: Text('$homeName  vs  $awayName', style: const TextStyle(color: AppTheme.textPrimary, fontSize: 13, fontWeight: FontWeight.w600))),
    ...
    return Column(children: [
      GestureDetector(
        onTap: () => setState(() { _selectedMatch = null; _playersHome = []; _playersAway = []; }),
        child: Row(children: [
          const Icon(Icons.arrow_back_ios_rounded, color: AppTheme.primary, size: 14),
          const SizedBox(width: 4),
          Text('F${_selectedMatch!['round']}  ·  $homeName vs $awayName', style: const TextStyle(color: AppTheme.primary, fontSize: 13, fontWeight: FontWeight.w600)),
        ]),
      ),
      const SizedBox(height: 16),
      TabBar(
      ...
      SizedBox(
        ...
        child: TabBarView(controller: _tabController, children: [
          ListView(children: _playersHome.map(_playerStatRow).toList()),
          ListView(children: _playersAway.map(_playerStatRow).toList()),
        ]),
      ),
      const SizedBox(height: 12),
      _saveButton('Guardar Acta', () => _showSuccess(context, 'Acta guardada exitosamente')),
    ]);
  }
}
